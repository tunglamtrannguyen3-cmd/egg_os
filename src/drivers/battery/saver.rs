...
impl BatterySaver {
    pub const fn new() -> Self {
        Self {
            enabled: false,
            auto_enable: true,
            threshold_percentage: 15, // 15% Low Battery Limit
            active_profile: PowerProfile::Balanced,
            prompt_state: PowerPromptState::None,
        }
    }

    /// Evaluates current power status and sets prompt state if <= 15%
    ...
    /// Processes keyboard inputs when responding to the centered low-battery prompt
